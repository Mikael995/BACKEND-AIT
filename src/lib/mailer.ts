...
import resend from "./resend"; // Import the default we just created

export const sendAITVerificationEmail = async (email: string, firstName: string, token: string) => {
  const verifyUrl = `${process.env.CLIENT_URL || 'http://localhost:8080'}/verify-email?token=${token}`;

  try {
    const result = await resend.emails.send({
      from: 'AIT Verification <no-reply@ivoriansintexas.com>',
      to: email,
        ...
        <html>
          <head>
            <style>
              ...
              .button { display: inline-block; background: #16a34a; color: #ffffff !important; padding: 18px 36px; border-radius: 16px; font-weight: 700; text-decoration: none; font-size: 16px; }
